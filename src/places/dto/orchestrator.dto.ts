...
export const OrchestratorQuerySchema = z.object({
  ...
  includeRawData: z
    .boolean()
    .optional()
    .describe('Whether to include raw tool outputs'),
});
...
export const OrchestratorResponseSchema = z.object({
  ...
  executionPlan: z
    .array(z.string())
    .describe('Steps taken to fulfill the query'),
  confidence: z
    .number()
    .min(0)
    .max(1)
    .describe('Confidence score for the response'),
...
export const OrchestratorStreamingResponseSchema = z.object({
  type: z
    ...
    .describe('Type of streaming update'),
...
export class OrchestratorQueryDto {
  @ApiProperty({
    ...
    example:
      'Find popular restaurants in Manhattan and tell me about foot traffic',
  })
  @IsString()
  query: string;
...
export class OrchestratorResponseDto {
  @ApiProperty({
    ...
    example:
      'Found 15 popular restaurants in Manhattan. Most have high foot traffic during lunch hours...',
  })
  summary: string;
